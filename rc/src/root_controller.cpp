 ...
 */
void
malleability_manager_run(margo_instance_id mid)
{
  int ret = register_rpc(mid, 0);
 ...
 */
void
slurm_manager_run(margo_instance_id mid)
{
  int ret = register_rpc(mid, 1);
 ...
 */
void
io_scheduler_run(margo_instance_id mid)
{
  int ret = register_rpc(mid, 2);
 ...
 */
void
adhoc_storage_manager_run(margo_instance_id mid)
{
  int ret = register_rpc(mid, 3);
 ...
 */
void
monitoring_manager_run(margo_instance_id mid)
{
  int ret = register_rpc(mid, 4);
...
main(int argc, char ** argv) {
  ...
  thread malleabMan(malleability_manager_run, mid);

  /*Malleability manager thread*/
  thread slurm(slurm_manager_run, mid);
